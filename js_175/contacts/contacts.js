...
// const doesContactExist = (firstName, lastName) => {
//   return contactData.some(contact => contact.firstName === firstName && contact.lastName === lastName);
// }
...
const validateName = (name, whichName) => {
  return body(name)
    ...
    .bail()
    .isLength({ max: 25 })
    .withMessage(`${whichName} name is too long. Max character length is 25 characters`)
    .isAlpha()
    .withMessage(`${whichName} name contains invalid characters. The name must be alphatical`);
}
